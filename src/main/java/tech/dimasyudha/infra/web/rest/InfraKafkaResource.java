...
import tech.dimasyudha.infra.config.KafkaSseConsumer;
import tech.dimasyudha.infra.config.KafkaSseProducer;

@RestController
@RequestMapping("/api/infra-kafka")
public class InfraKafkaResource {

    private final Logger log = LoggerFactory.getLogger(InfraKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public InfraKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
